...
The below instructions have been tested on Ubuntu 20.04 and on Raspbian 10 (Buster).
...
NOTE: When tested, the beacon transmit only partly worked on Raspberry Pi 3 and 4.
The pre-defined single location message that can be uncommented in the beacon.conf file works okay.
However, any data that the `transmit` program tries to write to the host ap daemon does not appear to be used.
...
When tested on a Raspberry Pi 3 and 4, the Extended Advertising HCI interface commands are not supported (`option 4` and `option 5 p`).
